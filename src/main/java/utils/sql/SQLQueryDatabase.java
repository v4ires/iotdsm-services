...
public class SQLQueryDatabase {
    ...
    public static String mySqlSensorSelectWithLimitAndOffsetQuery = "SELECT s.*, ss.name as sensor_source_name,ss.description as sensor_source_description,ss.create_time as sensor_source_create_time FROM tb_sensor s LEFT JOIN tb_sensor_source ss ON (s.sensor_source_id = ss.id) ORDER BY id LIMIT ? OFFSET ?";
    public static String mySqlSensorSelectWithOffsetQuery = "SELECT s.*, ss.name as sensor_source_name,ss.description as sensor_source_description,ss.create_time as sensor_source_create_time FROM tb_sensor s LEFT JOIN tb_sensor_source ss ON (s.sensor_source_id = ss.id) ORDER BY id LIMIT 18446744073709551615 OFFSET ?";
